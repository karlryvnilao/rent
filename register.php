    ...
    if (!empty($_FILES['profile_pic']['name'])) {
        ...
        if ($profile_pic_error === UPLOAD_ERR_OK) {
            if (!in_array($file_type, $allowed_types)) {
                ...
                $upload_dir = 'assets/img/profile_pic/';
        ...
        if ($password !== $confirm_password) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO user_details (user_id, firstname, lastname, birthday, address, contactnumber, profile_pic) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("issssss", $user_id, $firstname, $lastname, $birthday, $address, $contactnumber, $upload_path);
